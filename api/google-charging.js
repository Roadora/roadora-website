// Roadora Google Charging API — v4.1 Route Core Lock
// Server-side Google Places proxy for EV charging stations along an ORS route.
...
function normalizePlace(hit) {
  ...
  const name = place?.displayName?.text || 'Laadpunt';
...
export default async function handler(req, res) {
  const security = roadoraSecureRequest(req, res, { methods: 'POST, OPTIONS', maxRequests: 60, bucket: 'CHARGING' });

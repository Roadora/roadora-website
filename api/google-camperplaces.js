...
function buildHotelQuery() {
  return 'camperplaats motorhome stopover rv park campground campervan parking';
}
...
function inferHotelAmenities(place) {
  ...
  const amenities = new Set(['Camper']);
  if (text.includes('rv_park') || text.includes('camper')) amenities.add('Camperplek');
  if (text.includes('campground') || text.includes('camping')) amenities.add('Camping');
  if (text.includes('parking')) amenities.add('Parkeren');
...
function normalizePlace(hit) {
  ...
  const name = place?.displayName?.text || 'Camperplek langs route';
  ...
  return {
    ...
    status: openNow === true ? 'nu toegankelijk' : 'beschikbaarheid controleren',
...
export default async function handler(req, res) {
  const security = roadoraSecureRequest(req, res, { methods: 'POST, OPTIONS', maxRequests: 60, bucket: 'CAMPER' });
  ...
  const hotelQuery = buildHotelQuery();

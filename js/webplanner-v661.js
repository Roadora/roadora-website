...
function createDefaultState(){
  return {
    ...
    routeSource:'empty', routeDistanceKm:0, routeDurationMin:0, routeZones:[], placeStatus:{}, dayHotels:{}, routePreference:'fastest', routeVariantId:'', routeVariantSummaries:[], routeMismatchCount:0, foodType:'restaurant', outingType:'highlights'
  };
}
...
const categorySpecs = {
  ...
  uitjes: {
    ...
    sort:'Live bezienswaardigheden, natuur, cultuur en gezinsuitjes verspreid over de actieve reisdag',
    match:['Actieve reisdag', 'Route', 'Weinig omrijden'],
    why:['Ligt langs de actieve reisdag', 'Je kiest zelf het soort uitje', 'Stopduur wordt automatisch meegerekend']
  },
  wc: {
...
function resetLiveResultsForRoute(){
  ...
  ['uitjes'].forEach(cat=>setPlaceStatus(cat,'idle','Klik op Uitjes om live bezienswaardigheden en korte stops langs de actieve reisdag te zoeken.'));
}
...
const DISTRIBUTED_RESULT_CATEGORIES = new Set(['restaurants','uitjes']);
...
async function loadLivePlacesFor(cat){
  ...
  const points = isOvernightCategory(cat)
    ...
    : (isEnergyCategory(cat) ? energySearchPoints() : activeDayRouteSamplePoints((cat==='restaurants'||cat==='uitjes')?10:(cat==='wc'?8:6)));
  if(!points.length){
  ...
  try{
    const radiusMeters=overnight?22000:(cat==='restaurants'?5000:cat==='uitjes'?6500:cat==='wc'?4500:7000);
    ...
    if(list.length){
      stops[cat]=list;
      const message=supportsDistributedRecommendations(cat)
        ? `${list.length} resultaten verspreid over Dag ${state.activeDay}.`
        : (isOvernightCategory(cat)?`${list.length} resultaten binnen het gekozen aankomstgebied.`:`${list.length} resultaten langs Dag ${state.activeDay}.`);
...
function stopMatchLabels(cat, desc='', meta={}){
  ...
  if(supportsDistributedRecommendations(cat) && state.view==='recommended' && meta.distributionLabel){
    return [meta.distributionLabel, detour, stopWindow(cat)].slice(0,3);
  }
...
function stopWindow(cat){ const z=zoneForCategory(cat); if(cat==='restaurants') return `${foodTypeLabel()} · Dag ${state.activeDay}`; if(cat==='uitjes') return `${outingTypeLabel()} · Dag ${state.activeDay}`; if(cat==='wc') return `onderweg op Dag ${state.activeDay}`; return z ? (isOvernightCategory(cat) ? 'zelf gekozen' : `rond ${z.time}`) : ({hotels:'zelf gekozen',camperplaces:'zelf gekozen',laden:'rond 15:15',tanken:'rond 15:15'}[cat]||'onderweg');}
...
function renderStops(){
  ...
  const emptyHtml = status==='loading'
    ? `<div class="empty-stops"><strong>Live resultaten laden…</strong><span>Roadora zoekt verspreid over de actieve reisdag.</span></div>`
    : status==='error'
...
function addStopToActiveDay(cat,index){
  ...
  const type = {restaurants:foodTypeLabel(meta.foodType||state.foodType),laden:'Laden/tanken',tanken:'Laden/tanken',uitjes:meta.outingLabel||outingTypeLabel(meta.outingType||state.outingType),wc:'WC/pauze'}[cat]||'Stop';
...
function bind(){
  ...
  document.addEventListener('click',e=>{
    ...
    const cat=e.target.closest('[data-cat]'); if(cat){
      ...
      state.category=next;
      state.suggestions=supportsDistributedRecommendations(next);
      ...
      if(hasRoute()){
        ...
        } else if(next==='laden' || next==='tanken' || next==='restaurants' || next==='uitjes' || next==='wc'){
    ...
    if(foodFilter){
      ...
      state.suggestions=true; state.view='recommended';
    ...
    if(outingFilter){
      ...
      state.suggestions=true; state.view='recommended';
      resetLiveCategory('uitjes','loading');
      renderStops();
      loadLivePlacesFor('uitjes');
      saveDraftNow();
      toast(`${outingTypeLabel(nextType)} zoeken langs Dag ${state.activeDay}`);
      return;
    }

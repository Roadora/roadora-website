...
function createDefaultState(){
  return {
    ...
    routeSource:'empty', routeDistanceKm:0, routeDurationMin:0, routeZones:[], placeStatus:{}, dayHotels:{}, routePreference:'fastest', routeVariantId:'', routeVariantSummaries:[], routeMismatchCount:0, foodType:'restaurant', outingType:'highlights', tripType:'single', roundtripStops:[], daySettings:{}, foodSearchRadiusKm:10, foodSearchCenter:null, outingSearchMode:'place', outingSearchRadiusKm:25, outingSearchQuery:'', outingSearchTerm:'', outingAreaQuery:'', outingSearchCenter:null
  };
}
...
function setDepartTime(value){
  ...
  state.depart=val;
  ensureDaySettings(1).depart=val;
  const plan=timelines[1];if(plan?.[0])plan[0][0]=val||'—';
  updateDepartTimeDisplay();
  refreshAllTimelineTimes();
  renderAll();
  saveDraftNow();
}
function defaultDaySettings(day){
  const d=Math.max(1,Number(day)||1);
  return {
    depart:d===1?normalizeTimeValue(state.depart):'',
    desiredArrival:'',
    arrivalSlot:d===1?String(state.arrival||''):'',
    kind:'travel'
  };
}
function normalizeDaySettingsEntry(value,day){
  const base=defaultDaySettings(day);
  const source=value&&typeof value==='object'?value:{};
  return {
    depart:normalizeTimeValue(source.depart||base.depart),
    desiredArrival:normalizeTimeValue(source.desiredArrival||''),
    arrivalSlot:/^(12-14|14-16|16-18|18-20|20-22)$/.test(String(source.arrivalSlot||''))?String(source.arrivalSlot):base.arrivalSlot,
    kind:source.kind==='stay'?'stay':'travel'
  };
}
function ensureDaySettings(day=state.activeDay){
  const d=Math.max(1,Math.min(21,Number(day)||1));
  if(!state.daySettings||typeof state.daySettings!=='object') state.daySettings={};
  state.daySettings[d]=normalizeDaySettingsEntry(state.daySettings[d],d);
  return state.daySettings[d];
}
function normalizeAllDaySettings(){
  const next={};
  for(let d=1;d<=Math.max(1,Math.min(21,Number(state.days)||1));d++) next[d]=normalizeDaySettingsEntry(state.daySettings?.[d],d);
  state.daySettings=next;
  state.daySettings[1].depart=normalizeTimeValue(state.daySettings[1].depart||state.depart);
  state.daySettings[1].arrivalSlot=state.daySettings[1].arrivalSlot||String(state.arrival||'');
}
function activeDaySettings(){return ensureDaySettings(state.activeDay);}
function dayArrivalSlot(day=state.activeDay){return ensureDaySettings(day).arrivalSlot||'';}
function setDayDepart(day,value,{syncForm=true}={}){
  const d=Math.max(1,Number(day)||1), settings=ensureDaySettings(d);
  settings.depart=normalizeTimeValue(value);
  const plan=timelines[d];
  if(Array.isArray(plan)&&plan.length) plan[0][0]=settings.depart||'—';
  if(d===1){state.depart=settings.depart;if(syncForm&&$('#departTime'))$('#departTime').value=settings.depart;}
  refreshDayTimelineTimes(d,{sortStops:false});
  renderAll();saveDraftNow();
}
function setDayDesiredArrival(day,value){
  ensureDaySettings(day).desiredArrival=normalizeTimeValue(value);
  renderAll();saveDraftNow();
}
function setDayArrivalSlot(day,value,{syncForm=true}={}){
  const val=/^(12-14|14-16|16-18|18-20|20-22)$/.test(String(value||''))?String(value):'';
  ensureDaySettings(day).arrivalSlot=val;
  if(Number(day)===1){state.arrival=val;if(syncForm&&$('#hotelArrival'))$('#hotelArrival').value=val;}
  stops.hotels=[];stops.camperplaces=[];clearPlaceMarkers();
  setPlaceStatus('hotels','idle','Klik op Hotels om rond het gekozen aankomsttijdvak te zoeken. Kies eerst een aankomsttijdvak voor de overnachting.');
  setPlaceStatus('camperplaces','idle','Klik op Camperplekken om rond het gekozen aankomsttijdvak te zoeken. Kies eerst een aankomsttijdvak voor de overnachting.');
  renderAll();saveDraftNow();
}
function clockDifferenceMinutes(actual,desired){
  const a=minutesFromClock(actual),d=minutesFromClock(desired);if(a===null||d===null)return null;
  let diff=a-d;if(diff>720)diff-=1440;if(diff<-720)diff+=1440;return diff;
}
function dayFeasibility(day=state.activeDay){
  const settings=ensureDaySettings(day), plan=timelines[Number(day)]||[];
  const actual=safeTimeValue(plan.at(-1)?.[0]);
  if(!settings.desiredArrival)return {tone:'neutral',text:'Vul een gewenste aankomsttijd in om de haalbaarheid te controleren.'};
  if(!actual)return {tone:'neutral',text:`Gewenste aankomst ${settings.desiredArrival} · plan eerst deze reisdag.`};
  const diff=clockDifferenceMinutes(actual,settings.desiredArrival);
  if(diff===null)return {tone:'neutral',text:`Verwachte aankomst ${actual} · gewenst ${settings.desiredArrival}.`};
  if(diff>0)return {tone:'late',text:`Verwachte aankomst ${actual} · ${durationLabel(diff)} later dan gewenst.`};
  if(diff<0)return {tone:'early',text:`Verwachte aankomst ${actual} · ${durationLabel(Math.abs(diff))} eerder dan gewenst.`};
  return {tone:'ok',text:`Verwachte aankomst ${actual} · precies op je gewenste tijd.`};
}
...
function setFormFromState(){
  ...
  normalizeAllDaySettings();
  set('departTime', ensureDaySettings(1).depart || state.depart || '');
  set('hotelArrival', ensureDaySettings(1).arrivalSlot || state.arrival || '');
...
function applyDraftSnapshot(draft){
  ...
  // Compatibiliteitsmigratie voor oudere lokaal opgeslagen roadtrips.
  ...
  normalizeAllDaySettings();
  for(let d=1;d<=state.days;d++){const settings=ensureDaySettings(d);const plan=timelines[d];if(plan?.[0]){if(!settings.depart)settings.depart=safeTimeValue(plan[0][0]);else plan[0][0]=settings.depart||'—';}if(plan?.[0]?.[4]?.linkedFromPrevious)settings.linkedFromPrevious=true;}
  refreshAllTimelineTimes();
  setFormFromState();
...
const FEATURE_ROADORA_SUGGESTIONS = false; // Geen automatische route- of stopsuggesties; de gebruiker kiest zelf.
const timelines = {
...
function renderRouteChoices(){
  ...
  if(note){
    ...
    if(hasRoute()){
      const base=`Actief: ${routePreferenceLabel()} · ${routeProviderLabel()}. De routelijn loopt via je actieve geselecteerde stops.`;
      const mismatch=Number(state.routeMismatchCount)||0;
      const check=mismatch?` ${mismatch} ${mismatch===1?'stop kon':'stops konden'} nog niet in de nieuwe route worden verwerkt; probeer Route opnieuw berekenen.`:'';
...
function activeDayStartClock(day=state.activeDay){
  ...
  const plan=timelines[d] || [];
  return safeTimeValue(ensureDaySettings(d).depart) || safeTimeValue(plan[0]?.[0]) || (d===1?safeTimeValue(state.depart):'');
}
...
function hotelArrivalSearchPoints(){
  const startClock=activeDayStartClock();
  const depart=String(startClock||'').match(/^(\d{1,2}):(\d{2})$/);
  const slot=String(dayArrivalSlot(state.activeDay)||'').match(/^(\d{1,2})-(\d{1,2})$/);
...
function syncFollowingDayArrival(day){
  ...
  if(!safeTimeValue(plan[0]?.[0])) plan[0][0]=ensureDaySettings(d).depart || '';
...
function plannedStopView(row,day){
  ...
  const impact=Number.isFinite(Number(meta.routeImpactMinutes))?Number(meta.routeImpactMinutes):Number(meta.preAddDetourMinutes);
  const maxImpact=Math.max(0,Number(state.maxDetour)||20);
  const warning=meta.routeMismatch
    ...
    : (Number.isFinite(impact)&&impact>maxImpact ? `Deze stop voegt ongeveer ${durationLabel(Math.round(impact))} rijtijd toe` : '');
...
function refreshDayTimelineTimes(day=state.activeDay,{sortStops=true}={}){
  ...
  const startClock=safeTimeValue(ensureDaySettings(d).depart) || safeTimeValue(plan[0]?.[0]) || (d===1?safeTimeValue(state.depart):'');
  ...
  const manualOrderLocked=middle.some(row=>Boolean(row?.[4]?.manualOrderLocked));
  if(sortStops&&!manualOrderLocked){
  ...
  middle.forEach(row=>{
    ...
    const progress=routeProgressForMeta(meta);
    if(meta.skipped){
      if(startMinutes!==null && meta.autoTime!==false) row[0]='—';
      return;
    }
    if(startMinutes!==null && progress!==null && meta.autoTime!==false){
  ...
  const totalDuration=Math.round(finalDrive+accumulatedDelay);
  ...
  if(startMinutes!==null){
    ...
    ensureDaySettings(d).computedArrival=finalArrival;
    ...
    // Ook zonder vertrektijd moeten afstand en totale reisduur na een nieuwe route direct verversen.
    ensureDaySettings(d).computedArrival='';
    if(finalMeta.autoTime!==false) last[0]='—';
    if(selected){
      selected.info=selected.info||{};
      selected.info.arrivalTime='—';
      selected.info.progress=endProgress;
    }
  }
  if(selected){
    ...
    last[2]=`${segmentKm} km · ${durationLabel(totalDuration)}`;
  }
...
function isGoogleMapsUrl(value){
  try{const u=new URL(String(value||'').trim());return /(^|\.)(google\.[a-z.]+|maps\.app\.goo\.gl|goo\.gl)$/i.test(u.hostname)||/google\.[a-z.]+$/i.test(u.hostname)&&u.pathname.includes('/maps');}catch(_){return false;}
}
function googleMapsQueryFromUrl(value){
  try{
    const u=new URL(String(value||'').trim());
    for(const key of ['query','q','destination','daddr']){const val=u.searchParams.get(key);if(val&&!parseCoordinateText(val))return sanitizePlainText(decodeURIComponent(val.replace(/\+/g,' ')),180);}
    const place=u.pathname.match(/\/place\/([^/]+)/i);if(place?.[1])return sanitizePlainText(decodeURIComponent(place[1].replace(/\+/g,' ')),180);
  }catch(_){ }
  return '';
}
async function resolveGoogleMapsLink(value){
  const direct=parseCoordinateText(value);if(direct)return {...direct,label:'Google Maps-coördinaten',sourceUrl:String(value)};
  const query=googleMapsQueryFromUrl(value);if(query){const geo=await geocodePlace(query);return {lat:Number(geo.lat??geo.location?.lat??geo.coord?.[1]),lng:Number(geo.lng??geo.location?.lng??geo.coord?.[0]),label:geo.formattedAddress||query,placeId:geo.placeId||'',sourceUrl:String(value)};}
  const res=await fetch('/api/resolve-map-link?url='+encodeURIComponent(String(value||'')),{headers:{Accept:'application/json'}});
  const data=await res.json().catch(()=>({}));
  if(!res.ok||!data.ok||!data.result)throw new Error(data.message||'Google Maps-link kon niet worden gelezen');
  return {...data.result,sourceUrl:String(value)};
}
...
async function resolveSearchCenter(value){
  const raw=sanitizePlainText(value,400);
  const parsed=parseCoordinateText(raw);
  if(parsed) return {...parsed,label:`${Number(parsed.lat).toFixed(6)}, ${Number(parsed.lng).toFixed(6)}`};
  if(isGoogleMapsUrl(raw)) return resolveGoogleMapsLink(raw);
...
function handleMapSelectionClick(event){
  ...
  if(mapPickMode==='manual-place'){
    ...
    $('#manualPlaceLocation').value=value;
    manualResolvedCenter={...clicked,label:value,input:value};
    ...
    finishMapPick(); openManualPlaceModal({preserveAdvanced:true});
    manualResolvedCenter={...clicked,label:value,input:value};
    const name=sanitizePlainText($('#manualPlaceSearch')?.value,160)||'Zelf gekozen kaartpunt';
    if($('#manualPlaceName'))$('#manualPlaceName').value=name;
    const selection=$('#manualPlaceSelection');if(selection){selection.classList.remove('hidden');selection.setAttribute('aria-hidden','false');}
...
function renderRoundtripStops(){
  ...
  list.innerHTML=rows.map((value,index)=>`<div class="roundtrip-stop-row"><span>${index+1}</span><input type="text" data-roundtrip-stop="${index}" value="${escapeHtml(value)}" placeholder="Plaats of adres"><div class="roundtrip-stop-actions"><button type="button" data-move-roundtrip-stop="up" data-roundtrip-index="${index}" aria-label="Tussenbestemming omhoog" ${index===0?'disabled':''}>↑</button><button type="button" data-move-roundtrip-stop="down" data-roundtrip-index="${index}" aria-label="Tussenbestemming omlaag" ${index===rows.length-1?'disabled':''}>↓</button><button type="button" data-remove-roundtrip-stop="${index}" aria-label="Tussenbestemming verwijderen">×</button></div></div>`).join('');
}
function moveRoundtripStop(index,direction){
  const rows=Array.isArray(state.roundtripStops)?state.roundtripStops:[];
  const from=Number(index),to=direction==='up'?from-1:from+1;
  if(!Number.isInteger(from)||to<0||to>=rows.length)return;
  [rows[from],rows[to]]=[rows[to],rows[from]];
  renderRoundtripStops();saveDraftNow();toast('Volgorde rondreis aangepast');
}
function setManualPlaceStatus(text,tone='neutral'){
  const el=$('#manualPlaceStatus');if(!el)return;el.textContent=text;el.dataset.tone=tone;
}
function setManualSearchStatus(text,tone='neutral'){
  const el=$('#manualPlaceSearchStatus');if(!el)return;el.textContent=text;el.dataset.tone=tone;
}
function populateManualPlaceDays(){
  const select=$('#manualPlaceDaySelect');if(!select)return;
  const count=Math.max(1,Number(state.days)||1);
  select.innerHTML=Array.from({length:count},(_,i)=>`<option value="${i+1}" ${(i+1)===Number(state.activeDay)?'selected':''}>Dag ${i+1}</option>`).join('');
}
function manualSearchBias(){
  if(!hasRoute()||!Array.isArray(routeCoords)||!routeCoords.length)return null;
  const mid=routeCoords[Math.floor((routeCoords.length-1)/2)];
  const lng=Number(mid?.[0]),lat=Number(mid?.[1]);
  if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;
  const radius=Math.max(50000,Math.min(500000,Math.round(Math.max(80,Number(state.routeDistanceKm)||0)*650)));
  return {lat,lng,radius};
}
function manualTypeFromPlace(place={}){
  const types=new Set(Array.isArray(place.types)?place.types:[]);
  if(types.has('campground')||types.has('rv_park'))return 'camperplace';
  if(types.has('hotel')||types.has('lodging'))return 'hotel';
  if(types.has('restaurant')||types.has('cafe')||types.has('fast_food_restaurant')||types.has('bakery'))return 'food';
  if(types.has('tourist_attraction')||types.has('museum')||types.has('amusement_park')||types.has('zoo')||types.has('aquarium')||types.has('park')||types.has('national_park')||types.has('historical_landmark'))return 'outing';
  return 'stop';
}
function syncManualPlaceRoleAndType(source='role'){
  const role=$('#manualPlaceRole');const type=$('#manualPlaceType');if(!role||!type)return;
  if(source==='role'){
    if(role.value==='overnight'&&!['hotel','camperplace'].includes(type.value)) type.value=supportsCamperPlaces()?'camperplace':'hotel';
    if(role.value==='stop'&&['hotel','camperplace'].includes(type.value)) type.value='stop';
  }else{
    role.value=['hotel','camperplace'].includes(type.value)?'overnight':'stop';
  }
}
function clearManualPlaceSelection(){
  manualSelectedPlace=null;
  manualResolvedCenter=null;
  const selection=$('#manualPlaceSelection');if(selection){selection.classList.add('hidden');selection.setAttribute('aria-hidden','true');}
  if($('#manualPlaceName'))$('#manualPlaceName').value='';
  const save=$('#saveManualPlace');if(save)save.disabled=true;
}
function resetManualPlaceForm({preserveAdvanced=false}={}){
  manualPlaceResults=[];
  clearManualPlaceSelection();
  const results=$('#manualPlaceResults');if(results)results.innerHTML='';
  if(!preserveAdvanced){
    if($('#manualPlaceSearch'))$('#manualPlaceSearch').value='';
    if($('#manualPlaceLocation'))$('#manualPlaceLocation').value='';
    if($('#manualPlaceNotes'))$('#manualPlaceNotes').value='';
    setManualPlaceStatus('Nog geen geavanceerde locatie gecontroleerd.');
    setManualSearchStatus('Vul een naam of adres in.');
  }
  populateManualPlaceDays();
}
function openManualPlaceModal(options={}){
  const modal=$('#manualPlaceModal');if(!modal)return;
  resetManualPlaceForm(options);
  modal.classList.add('open');modal.setAttribute('aria-hidden','false');
  setTimeout(()=>$('#manualPlaceSearch')?.focus(),40);
}
function closeManualPlaceModal(){
  const modal=$('#manualPlaceModal');if(!modal)return;
  modal.classList.remove('open');modal.setAttribute('aria-hidden','true');finishMapPick();
}
function manualResultMeta(place,index){
  const rating=Number(place?.rating);
  const bits=[place?.typeLabel||'Locatie'];
  if(Number.isFinite(rating))bits.push(`${rating.toFixed(1).replace('.0','')} ★`);
  if(Number.isFinite(Number(place?.userRatingCount)))bits.push(`${Number(place.userRatingCount).toLocaleString('nl-NL')} beoordelingen`);
  if(typeof place?.openNow==='boolean')bits.push(place.openNow?'open bij laatste controle':'mogelijk gesloten');
  return bits.filter(Boolean).join(' · ');
}
function renderManualPlaceResults(){
  const root=$('#manualPlaceResults');if(!root)return;
  root.innerHTML=manualPlaceResults.map((place,index)=>`<article class="manual-place-result ${manualSelectedPlace?.id===place.id?'selected':''}">
    <div><strong>${escapeHtml(place.name||'Gevonden plek')}</strong><span>${escapeHtml(place.address||'Adres niet beschikbaar')}</span><small>${escapeHtml(manualResultMeta(place,index))}</small></div>
    <button class="btn ${manualSelectedPlace?.id===place.id?'primary':''}" type="button" data-select-manual-place="${index}">${manualSelectedPlace?.id===place.id?'Gekozen':'Kies'}</button>
  </article>`).join('');
}
async function searchManualPlaces(){
  const query=sanitizePlainText($('#manualPlaceSearch')?.value,160);
  if(query.length<2){setManualSearchStatus('Vul minimaal twee tekens in.','error');return;}
  clearManualPlaceSelection();manualPlaceResults=[];renderManualPlaceResults();
  setManualSearchStatus('Plekken zoeken…','loading');
  try{
    const params=new URLSearchParams({q:query});
    const bias=manualSearchBias();if(bias){params.set('lat',String(bias.lat));params.set('lng',String(bias.lng));params.set('radius',String(bias.radius));}
    const response=await fetch('/api/google-place-search?'+params.toString(),{headers:{Accept:'application/json'}});
    const data=await response.json().catch(()=>({}));
    if(!response.ok||!data.ok)throw new Error(data.message||'Zoeken is tijdelijk niet gelukt');
    manualPlaceResults=Array.isArray(data.results)?data.results:[];
    renderManualPlaceResults();
    setManualSearchStatus(manualPlaceResults.length?`${manualPlaceResults.length} mogelijke plekken gevonden. Kies de juiste.`:'Geen plek gevonden. Probeer een specifiekere naam of gebruik Meer opties.',manualPlaceResults.length?'ok':'error');
  }catch(err){setManualSearchStatus(String(err?.message||err),'error');}
}
function selectManualPlace(index){
  const place=manualPlaceResults[Number(index)];if(!place)return;
  manualSelectedPlace=cloneJsonSafe(place);
  manualResolvedCenter={lat:Number(place.lat),lng:Number(place.lng),label:place.address||place.name,placeId:place.id||'',input:`place:${place.id||index}`};
  if($('#manualPlaceName'))$('#manualPlaceName').value=place.name||'';
  if($('#manualPlaceLocation'))$('#manualPlaceLocation').value='';
  const type=manualTypeFromPlace(place);if($('#manualPlaceType'))$('#manualPlaceType').value=type;
  syncManualPlaceRoleAndType('type');
  const selection=$('#manualPlaceSelection');if(selection){selection.classList.remove('hidden');selection.setAttribute('aria-hidden','false');}
  setText('#manualPlaceSelectedName',place.name||'Gekozen plek');setText('#manualPlaceSelectedAddress',place.address||'');
  const save=$('#saveManualPlace');if(save)save.disabled=false;
  renderManualPlaceResults();showSearchCenter(place,'Gekozen plek');
  setManualSearchStatus('Plek gekozen. Kies hoe en op welke reisdag je hem wilt toevoegen.','ok');
}
async function checkManualPlaceLocation(){
  const location=sanitizePlainText($('#manualPlaceLocation')?.value,400);if(!location){toast('Vul eerst een Google Maps-link of coördinaten in');return null;}
  setManualPlaceStatus('Locatie controleren…','loading');
  try{
    const center=await resolveSearchCenter(location);
    if(!Number.isFinite(Number(center.lat))||!Number.isFinite(Number(center.lng)))throw new Error('Geen geldige coördinaten gevonden');
    manualSelectedPlace=null;
    manualResolvedCenter={...center,input:location};
    const name=sanitizePlainText($('#manualPlaceSearch')?.value,160)||center.label||'Zelf gekozen plek';
    if($('#manualPlaceName'))$('#manualPlaceName').value=name;
    const projection=hasRoute()?nearestRouteProjection(center.lat,center.lng):null;
    const routeText=projection?` · ${Math.round(projection.gapKm*10)/10} km van de huidige route`:'';
    setManualPlaceStatus(`Gevonden: ${center.label||`${Number(center.lat).toFixed(6)}, ${Number(center.lng).toFixed(6)}`}${routeText}`,'ok');
    const selection=$('#manualPlaceSelection');if(selection){selection.classList.remove('hidden');selection.setAttribute('aria-hidden','false');}
    setText('#manualPlaceSelectedName',name);setText('#manualPlaceSelectedAddress',center.label||location);
    const save=$('#saveManualPlace');if(save)save.disabled=false;
    showSearchCenter(center,'Zelf gekozen plek');
    return manualResolvedCenter;
  }catch(err){manualResolvedCenter=null;setManualPlaceStatus(String(err?.message||err),'error');toast('Locatie kon niet worden gecontroleerd');return null;}
}
function manualCategoryFromType(type,role='stop'){
  if(role==='overnight')return type==='camperplace'?'camperplaces':'hotels';
  return ({food:'restaurants',outing:'uitjes',stop:'wc',other:'wc'})[type]||'wc';
}
function manualLabelFromType(type,role='stop'){
  if(role==='overnight')return type==='camperplace'?'Camperplek':'Hotel';
  return ({food:'Eten',outing:'Uitje',stop:'Tussenstop',other:'Zelf ingevuld'})[type]||'Zelf ingevuld';
}
async function saveManualPlace(){
  const role=$('#manualPlaceRole')?.value==='overnight'?'overnight':'stop';
  let type=$('#manualPlaceType')?.value||'stop';
  if(role==='overnight'&&!['hotel','camperplace'].includes(type))type=supportsCamperPlaces()?'camperplace':'hotel';
  if(role==='stop'&&['hotel','camperplace'].includes(type))type='stop';
  const notes=sanitizePlainText($('#manualPlaceNotes')?.value,300);
  const targetDay=Math.max(1,Math.min(Number(state.days)||1,Number($('#manualPlaceDaySelect')?.value)||state.activeDay));
  let place=manualSelectedPlace?cloneJsonSafe(manualSelectedPlace):null;
  let center=manualResolvedCenter;
  if(!place&&(!center||!Number.isFinite(Number(center.lat))||!Number.isFinite(Number(center.lng)))){toast('Zoek en kies eerst een plek');return;}
  if(!center)center={lat:Number(place.lat),lng:Number(place.lng),label:place.address||place.name,placeId:place.id||''};
  const name=sanitizePlainText(place?.name||$('#manualPlaceName')?.value||$('#manualPlaceSearch')?.value,120)||'Zelf gekozen plek';
  const projection=hasRoute()?nearestRouteProjection(center.lat,center.lng):null;
  const cat=manualCategoryFromType(type,role);
  const label=manualLabelFromType(type,role);
  const estimate=projection?Math.max(2,Math.min(180,Math.round(2+projection.gapKm*2.2))):0;
  const meta={
    id:`user-${Date.now()}`,source:'user',manualLabel:label,userAdded:true,provider:place?.provider||'Zelf toegevoegd',
    lat:Number(center.lat),lng:Number(center.lng),placeId:place?.id||center.placeId||'',name,category:cat,address:place?.address||center.label||'',notes,
    sourceUrl:place?.googleMapsUri||'',googleMapsUri:place?.googleMapsUri||`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${center.lat},${center.lng}`)}`,
    website:place?.website||'',phone:place?.phone||'',rating:place?.rating??null,userRatingCount:place?.userRatingCount??null,openNow:place?.openNow??null,
    status:'controleer toegang, opening en beschikbaarheid bij de aanbieder',amenities:['Zelf gekozen'],
    stopDurationMin:type==='food'?45:type==='outing'?60:role==='overnight'?0:10,
    detourMinutes:estimate,detourLabel:estimate?`geschat ± ${estimate} min extra`:''
  };
  if(projection){meta.routeProgress=clampRouteProgress(projection.progress);meta.distanceFromStartMeters=Math.round(meta.routeProgress*Number(state.routeDistanceKm||0)*1000);meta.routeGapKm=Math.round(projection.gapKm*10)/10;meta.preAddDetourMinutes=estimate;}
  const desc=['Zelf gekozen',notes,meta.detourLabel].filter(Boolean).join(' · ');
  stops[cat].push([name,desc,meta]);
  const index=stops[cat].length-1;
  state.activeDay=targetDay;state.category=cat;manualResolvedCenter=null;manualSelectedPlace=null;closeManualPlaceModal();
  await addStopToActiveDay(cat,index);
}
...
function linkPreviousOvernightToDay(day=state.activeDay){
  ...
  const settings=ensureDaySettings(d);
  const plan=timelines[d]||dayPlan(); const meta={...navMetaForHotel(previous),linkedFromPrevious:true};
  plan[0]=[settings.depart||safeTimeValue(plan[0]?.[0])||'',`Vertrek ${previous.name}`,'Door jou gekoppeld aan de overnachting van de vorige dag','Vertrek',meta];
...
const ROUTE_WAYPOINT_LIMIT=25;
function routeCoordKey(coord){
  return Array.isArray(coord)&&coord.length===2?`${Number(coord[0]).toFixed(5)},${Number(coord[1]).toFixed(5)}`:'';
}
function plannedRouteWaypointCandidates(){
  const candidates=[];
  const dayCount=Math.max(1,Number(state.days)||1);
  for(let day=1;day<=dayCount;day++){
    const plan=Array.isArray(timelines[day])?timelines[day]:[];
    const startProgress=dayStartProgress(day), endProgress=dayEndProgress(day);
    const span=Math.max(0.0001,endProgress-startProgress);
    plan.forEach((row,index)=>{
      if(index===0 || row?.[4]?.skipped) return;
      const meta=row?.[4]||{};
      const lat=Number(meta.lat),lng=Number(meta.lng);
      if(!Number.isFinite(lat)||!Number.isFinite(lng)) return;
      let progress=Number(meta.routeProgress);
      if(!Number.isFinite(progress)){
        const projection=hasRoute()?nearestRouteProjection(lat,lng):null;
        progress=Number.isFinite(Number(projection?.progress))?Number(projection.progress):startProgress+(index/Math.max(1,plan.length-1))*span;
      }
      progress=Math.max(startProgress,Math.min(endProgress,progress));
      candidates.push({
        coord:[lng,lat],progress,order:day*1000+index,source:'planned',
        name:String(meta.name||row?.[1]||'Geselecteerde stop')
      });
    });
  }
  return candidates;
}
function mergeRouteWaypointCandidates(anchorGeocodes=[]){
  const anchors=(Array.isArray(anchorGeocodes)?anchorGeocodes:[]).map((geo,index)=>{
    const coord=Array.isArray(geo?.coord)?geo.coord.map(Number):null;
    if(!coord||coord.length!==2||!coord.every(Number.isFinite)) return null;
    const projection=hasRoute()?nearestRouteProjection(Number(coord[1]),Number(coord[0])):null;
    return {coord,progress:Number.isFinite(Number(projection?.progress))?Number(projection.progress):null,order:(index+1)*1000-500,source:'anchor',name:String(geo?.formattedAddress||'Rondreisbestemming')};
  }).filter(Boolean);
  const all=[...anchors,...plannedRouteWaypointCandidates()];
  all.sort((a,b)=>{
    const orderDiff=Number(a.order)-Number(b.order);
    if(Number.isFinite(orderDiff)&&orderDiff!==0)return orderDiff;
    const ap=Number(a.progress),bp=Number(b.progress);
    if(Number.isFinite(ap)&&Number.isFinite(bp)&&Math.abs(ap-bp)>0.000001)return ap-bp;
    return 0;
  });
  const seen=new Set();
  return all.filter(item=>{
    const key=routeCoordKey(item.coord);if(!key||seen.has(key)) return false;seen.add(key);return true;
  });
}
function routeWaypointCount(){return mergeRouteWaypointCandidates([]).filter(x=>x.source==='planned').length;}
let plannedRouteRecalcQueue=Promise.resolve(false);
function recalculateRouteThroughSelectedStops(reason='wijziging'){
  if(!hasRoute()) return Promise.resolve(false);
  const task=async()=>{
    toast('Route aanpassen via je geselecteerde stops…');
    const ok=await loadRealRoute({preservePlan:true});
    if(ok){
      const count=routeWaypointCount();
      toast(count?`Route opnieuw berekend via ${count} ${count===1?'geselecteerde stop':'geselecteerde stops'}`:'Route opnieuw berekend zonder tussenstops');
    } else {
      toast('Route niet aangepast; je planning blijft bewaard');
    }
    return ok;
  };
  plannedRouteRecalcQueue=plannedRouteRecalcQueue.catch(()=>false).then(task);
...
async function loadRealRoute({preservePlan=false,requestedPreference=null}={}){
  ...
  try{
    ...
    const startGeo=geocoded[0],endGeo=geocoded[geocoded.length-1],anchorGeocodes=geocoded.slice(1,-1);
    const waypointCandidates=mergeRouteWaypointCandidates(anchorGeocodes);
...
window.RoadoraPlanner.recalculateThroughStops=()=>recalculateRouteThroughSelectedStops('handmatige test');
window.RoadoraPlanner.plannedWaypoints=()=>plannedRouteWaypointCandidates().map(x=>x.coord);
window.RoadoraPlanner.routeCoordinates=()=>cloneJsonSafe(routeCoords);
...
function readForm(){
  ...
  ensureDaySettings(1).depart=state.depart;
  ensureDaySettings(1).arrivalSlot=state.arrival;
  state.activeDay=Math.max(1,Math.min(state.days,Number(state.activeDay)||1));
...
function rebuildDayConnections(){
  for(let day=1;day<=Number(state.days||1);day++){
    ...
    if(day===1){const settings=ensureDaySettings(1);plan[0]=[settings.depart||safeTimeValue(plan[0]?.[0])||'',`Vertrek ${dayStartName(1)}`,'Start van deze reisdag','Vertrek',plan[0]?.[4]||{}];}
    else if(plan[0]?.[4]?.linkedFromPrevious){
      const previous=selectedHotelForDay(day-1);
      if(previous) plan[0]=[ensureDaySettings(day).depart||safeTimeValue(plan[0]?.[0])||'',`Vertrek ${previous.name}`,'Door jou gekoppeld aan de overnachting van de vorige dag','Vertrek',{...navMetaForHotel(previous),linkedFromPrevious:true}];
      else plan[0]=[ensureDaySettings(day).depart||safeTimeValue(plan[0]?.[0])||'',`Vertrek dag ${day}`,'Kies zelf het vertrekpunt van deze reisdag','Vertrek',{}];
    }
...
function arrivalSlotOptions(selected=''){
  const options=[['','Geen tijdvak'],['12-14','12:00 - 14:00'],['14-16','14:00 - 16:00'],['16-18','16:00 - 18:00'],['18-20','18:00 - 20:00'],['20-22','20:00 - 22:00']];
  return options.map(([value,label])=>`<option value="${value}" ${value===selected?'selected':''}>${label}</option>`).join('');
}
function renderActiveDayControls(){
  const wrap=$('#activeDayControls');if(!wrap)return;
  const day=Number(state.activeDay)||1, settings=ensureDaySettings(day), feasibility=dayFeasibility(day);
  const previous=day>1?selectedHotelForDay(day-1):null;
  const plan=timelines[day]||dayPlan();
  const linked=Boolean(plan?.[0]?.[4]?.linkedFromPrevious);
  const hotel=selectedHotelForDay(day);
  wrap.innerHTML=`<div class="active-day-time-grid">
    <label>Vertrektijd Dag ${day}<input id="activeDayDepart" type="time" value="${escapeHtml(settings.depart||'')}"></label>
    <label>Gewenste aankomsttijd<input id="activeDayDesiredArrival" type="time" value="${escapeHtml(settings.desiredArrival||'')}"></label>
    <label>Aankomsttijdvak overnachting<select id="activeDayArrivalSlot">${arrivalSlotOptions(settings.arrivalSlot||'')}</select></label>
  </div>
  <div class="day-feasibility ${escapeHtml(feasibility.tone)}">${escapeHtml(feasibility.text)}</div>
  <div class="active-day-control-actions">
    <button class="btn" type="button" id="addDayAfter">+ Reisdag na Dag ${day}</button>
    ${hotel?`<button class="btn" type="button" id="addStayDay">+ Verblijfsdag bij ${escapeHtml(hotel.name)}</button>`:''}
    ${previous&&!linked?`<button class="btn" type="button" data-link-previous-overnight="${day}">Gebruik ${escapeHtml(previous.name)} als vertrek</button>`:''}
    ${linked?`<button class="btn" type="button" id="unlinkPreviousOvernight">Ontkoppel vorige overnachting</button>`:''}
  </div>
  <p class="hint">Roadora vult niets automatisch in. Jij kiest de tijden, koppelt dagen en voegt verblijfsdagen zelf toe.</p>`;
}
function shiftDayDataAfter(day,amount=1){
  const from=Math.max(0,Number(day)||0), step=Math.max(1,Number(amount)||1);
  for(let d=Number(state.days)||1;d>from;d--){
    if(timelines[d]){timelines[d+step]=timelines[d];delete timelines[d];}
    if(state.dayHotels?.[d]){state.dayHotels[d+step]=state.dayHotels[d];delete state.dayHotels[d];}
    if(state.daySettings?.[d]){state.daySettings[d+step]=state.daySettings[d];delete state.daySettings[d];}
  }
}
function insertTripDayAfter(day=state.activeDay,{stay=false}={}){
  const after=Math.max(1,Math.min(Number(state.days)||1,Number(day)||1));
  if(Number(state.days)>=21){toast('Je kunt maximaal 21 reisdagen opslaan');return false;}
  const hotel=selectedHotelForDay(after);
  if(stay&&!hotel){toast('Kies eerst een overnachtingsplek voor deze dag');return false;}
  shiftDayDataAfter(after,1);
  state.days=Number(state.days)+1;
  const newDay=after+1;
  if(stay){
    const nav={...navMetaForHotel(hotel),linkedFromPrevious:true,stayDay:true};
    timelines[newDay]=[
      ['',`Verblijf ${hotel.name}`,'Door jou toegevoegde verblijfsdag','Vertrek',nav],
      ['',`Overnachting ${hotel.name}`,'Geen lange dagroute gepland',hotel.category==='hotels'?'Hotel':'Camperplek',nav]
    ];
    state.dayHotels[newDay]=cloneJsonSafe(hotel);
    state.daySettings[newDay]={depart:'',desiredArrival:'',arrivalSlot:ensureDaySettings(after).arrivalSlot||'',kind:'stay'};
  }else{
    timelines[newDay]=[['',`Vertrek dag ${newDay}`,'Kies zelf het vertrekpunt van deze reisdag','Vertrek'],['',`Aankomst eindpunt Dag ${newDay}`,'Kies zelf een bestemming of overnachting','Bestemming']];
    state.daySettings[newDay]=defaultDaySettings(newDay);
  }
  ...
  toast(stay?`Verblijfsdag toegevoegd bij ${hotel.name}`:`Dag ${newDay} toegevoegd`);
  return true;
}
function unlinkPreviousOvernightFromDay(day=state.activeDay){
  const d=Number(day)||1;if(d<=1)return;
  const plan=timelines[d]||dayPlan();
  const settings=ensureDaySettings(d);
  settings.kind='travel';
  plan[0]=[settings.depart||'',`Vertrek dag ${d}`,'Kies zelf het vertrekpunt van deze reisdag','Vertrek',{}];
  refreshDayTimelineTimes(d,{sortStops:false});renderAll();saveDraftNow();toast(`Vertrek van Dag ${d} ontkoppeld`);
}
function refreshExplicitLinkedNextDay(day){
  const next=Number(day)+1;
  const plan=timelines[next];if(!Array.isArray(plan)||!plan.length||!plan[0]?.[4]?.linkedFromPrevious)return;
  const hotel=selectedHotelForDay(day);
  if(!hotel){
    plan[0]=[ensureDaySettings(next).depart||'',`Vertrek dag ${next}`,'Kies zelf het vertrekpunt van deze reisdag','Vertrek',{}];
    return;
  }
  plan[0]=[ensureDaySettings(next).depart||safeTimeValue(plan[0]?.[0])||'',`Vertrek ${hotel.name}`,'Door jou gekoppeld aan de overnachting van de vorige dag','Vertrek',{...navMetaForHotel(hotel),linkedFromPrevious:true}];
}
...
function dayStatus(day){
  if(timelines[day]) return ensureDaySettings(day).kind==='stay'?'verblijfsdag':'handmatig gepland';
...
function dayDataExistsBeyond(limit){
  return Object.keys(timelines).some(key=>Number(key)>limit && Array.isArray(timelines[key]) && timelines[key].length) ||
    Object.keys(state.dayHotels||{}).some(key=>Number(key)>limit) || Object.keys(state.daySettings||{}).some(key=>Number(key)>limit);
}
function pruneTripDataToDayCount(limit){
  ...
  Object.keys(state.daySettings||{}).forEach(key=>{ if(Number(key)>maxDay) delete state.daySettings[key]; });
  state.days=maxDay;
...
function setTripDayCount(value,{confirmRemoval=true,activateNew=false}={}){
  ...
  else {state.days=next;for(let d=previous+1;d<=next;d++)ensureDaySettings(d);}
...
function deleteTripDay(day){
  ...
  const nextSettings={};
  Object.entries(state.daySettings||{}).forEach(([key,value])=>{
    const current=Number(key);if(current===day)return;nextSettings[current>day?current-1:current]=value;
  });
...
function dayPlan(){
  if(!timelines[state.activeDay]){
    ...
    const startTime = activeDayStartClock(state.activeDay) || '—';
...
function planTypeSelect(type){
  ...
  const custom=current && !opts.includes(current) ? `<option selected>${escapeHtml(current)}</option>` : '';
  return `<select class="plan-type" aria-label="Type stop">${custom}${opts.map(o=>`<option ${o===current?'selected':''}>${o}</option>`).join('')}</select>`;
}
function isProtectedDestinationRow(row,index,list=dayPlan()){
  const i=Number(index);
  if(!Number.isInteger(i) || i!==list.length-1) return false;
  if(selectedHotelForDay(state.activeDay)) return false;
  const title=String(row?.[1]||'').trim();
  const type=String(row?.[3]||'').trim().toLowerCase();
  return type==='bestemming' || /^aankomst(?:\s|$)/i.test(title);
}
function canRemovePlanRow(row,index,list=dayPlan()){
  const i=Number(index);
  if(!Number.isInteger(i) || i<=0 || i>=list.length) return false;
  return !isProtectedDestinationRow(row,i,list);
}
function restoreDestinationRowWhenNeeded(plan){
  if(!Array.isArray(plan) || plan.length===0 || !state.destination) return;
  const last=plan[plan.length-1];
  if(isProtectedDestinationRow(last,plan.length-1,plan) || selectedHotelForDay(state.activeDay)) return;
  const destination=(state.destination||'').split(',')[0].trim() || 'bestemming';
  plan.push(['—',`Aankomst ${destination}`,shortRouteDetailForDay(state.activeDay),'Bestemming',{}]);
}
...
async function removePlanRow(index){
  const plan=dayPlan();
  const i=Number(index);
  ...
  const row=plan[i];
  if(isProtectedDestinationRow(row,i,plan)){toast('Het eindpunt van een reisdag kan niet worden verwijderd'); return false;}
  const label=String(row?.[1]||'deze stop').replace(/^Aankomst\s+/i,'').trim()||'deze stop';
  if(!confirm(`Wil je ${label} uit Dag ${state.activeDay} verwijderen?`)) return false;
  const hotel=selectedHotelForDay(state.activeDay);
  const isSelectedOvernight=i===plan.length-1 && Boolean(hotel);
  if(isSelectedOvernight){
    delete state.dayHotels[state.activeDay];
    refreshExplicitLinkedNextDay(state.activeDay);
    ...
    plan.splice(i,1);
    restoreDestinationRowWhenNeeded(plan);
  }
  ...
  await recalculateRouteThroughSelectedStops('stop verwijderd');
  return true;
}
function lockManualPlanOrder(plan=dayPlan()){
  plan.slice(1,-1).forEach((row,index)=>{
    if(!row[4]||typeof row[4]!=='object')row[4]={};
    row[4].manualOrderLocked=true;
    row[4].manualOrderIndex=index;
  });
}
async function movePlanStop(index,direction){
  const plan=dayPlan();const i=Number(index);
  if(!Number.isInteger(i)||i<=0||i>=plan.length-1){toast('Alleen tussenstops kunnen worden verplaatst');return false;}
  const target=direction==='up'?i-1:i+1;
  if(target<=0||target>=plan.length-1)return false;
  [plan[i],plan[target]]=[plan[target],plan[i]];
  lockManualPlanOrder(plan);
  refreshDayTimelineTimes(state.activeDay,{sortStops:false});
  renderTimeline();renderTripOverview();scheduleAutosave();
  toast('Stopvolgorde aangepast');
  await recalculateRouteThroughSelectedStops('stopvolgorde aangepast');
  return true;
}
function planRowByStopId(stopId){
  const id=String(stopId||'');if(!id)return null;
  for(const [dayKey,plan] of Object.entries(timelines)){
    if(!Array.isArray(plan))continue;
    const row=plan.find(item=>String(item?.[4]?.id||item?.[4]?.placeId||'')===id);
    if(row)return {day:Number(dayKey),row};
  }
  return null;
}
function applyRouteImpactToStop(stopId,beforeDuration){
  const found=planRowByStopId(stopId);if(!found)return;
  const after=Number(state.routeDurationMin)||0;
  const before=Number(beforeDuration)||0;
  const impact=Math.max(0,Math.round(after-before));
  if(!found.row[4]||typeof found.row[4]!=='object')found.row[4]={};
  found.row[4].routeImpactMinutes=impact;
  found.row[4].routeImpactCheckedAt=new Date().toISOString();
  refreshDayTimelineTimes(found.day,{sortStops:false});
  if(Number(found.day)===Number(state.activeDay)){renderTimeline();renderTripOverview();}
  scheduleAutosave();
}
function confirmEstimatedDetour(meta={},title='deze stop'){
  const estimate=Number.isFinite(Number(meta.preAddDetourMinutes))?Number(meta.preAddDetourMinutes):Number(meta.detourMinutes);
  const limit=Math.max(0,Number(state.maxDetour)||20);
  if(!Number.isFinite(estimate)||estimate<=limit||meta.detourConfirmed)return true;
  const accepted=confirm(`${title} ligt duidelijk buiten je huidige route en kan ongeveer ${durationLabel(Math.round(estimate))} extra rijtijd geven. Toch toevoegen?`);
  if(accepted)meta.detourConfirmed=true;
  return accepted;
}
...
async function toggleSkipPlannedStop(index){
  const plan=dayPlan();
  const i=Number(index);
  ...
  await recalculateRouteThroughSelectedStops(row[4].skipped?'stop overgeslagen':'stop hervat');
  return true;
}
function renderTimeline(){
  ...
  $('#timeline').innerHTML = list.map((r,i)=>{
    ...
    const removable=canRemovePlanRow(r,i,list);
    return `<div class="plan-row ${i===list.length-1?'active':''} ${editing?'editing':''} ${mismatch?'route-mismatch':''} ${skipped?'plan-row-skipped':''}" data-plan-index="${i}">
      <div class="plan-read plan-read-v653">
        ...
        <div class="plan-read-content">
          ...
          <div class="plan-read-actions">
            ${intermediate?`<span class="plan-order-actions" aria-label="Stopvolgorde"><button type="button" data-move-plan-stop="up" data-plan-move-index="${i}" aria-label="Zet ${escapeHtml(r[1])} eerder" ${i===1?'disabled':''}>↑</button><button type="button" data-move-plan-stop="down" data-plan-move-index="${i}" aria-label="Zet ${escapeHtml(r[1])} later" ${i===list.length-2?'disabled':''}>↓</button></span>`:''}
            ...
            ${removable?'<button class="plan-remove" type="button" title="Stop verwijderen">Verwijderen</button>':''}
          </div>
        </div>
      </div>
      <div class="plan-edit-panel">
        ...
        <div class="plan-fields">
          <div class="plan-topline">${planTypeSelect(type)}${removable?'<button class="plan-remove" type="button" title="Stop verwijderen">Verwijderen</button>':''}</div>
...
function openStopDetail(cat,index){
  ...
  const modalNavigate=$('#modalNavigateStop'); if(modalNavigate) modalNavigate.hidden=!['restaurants','uitjes'].includes(cat);
...
async function addStopToActiveDay(cat,index){
  const item=(stops[cat]||[])[index]; if(!item) return;
  const title=item[0];
  const desc=typeof item[1]==='function'?item[1]():item[1];
  const meta=item[2]||{};
  const plan=dayPlan();
  const routeDurationBefore=Number(state.routeDurationMin)||0;
  if(!confirmEstimatedDetour(meta,title))return;
  if(isOvernightCategory(cat)){
    ...
    const startMeta=plan[0]?.[4]&&typeof plan[0][4]==='object'?cloneJsonSafe(plan[0][4]):{};
    timelines[state.activeDay]=[
      [activeDayStartClock(state.activeDay) || '', `Vertrek ${start}`, plan[0]?.[2]||'Start van deze reisdag', 'Vertrek',startMeta],
      ...middle,
      [arrivalTime, `Aankomst ${title}`, shortRouteDetailForDay(state.activeDay), overnightType,navMeta]
    ];
    // Alleen een eerder bewust gekoppelde volgende dag wordt bijgewerkt.
    refreshExplicitLinkedNextDay(state.activeDay);
    refreshDayTimelineTimes(state.activeDay);
    if(state.activeDay < state.days) refreshDayTimelineTimes(state.activeDay+1);
    editingPlanRows.clear();
    ...
    const recalculated=await recalculateRouteThroughSelectedStops('overnachting toegevoegd');
  ...
  const selectedOutsideOldDay=routeProgress!==null && (routeProgress < startProgress-0.002 || routeProgress > endProgress+0.002);
  const navMeta={
    ...meta,
    ...
    autoTime:true,selectedOutsideOldDay
  };
  ...
  const recalculated=await recalculateRouteThroughSelectedStops(Number.isInteger(replacement)?'stop vervangen':'stop toegevoegd');
  if(recalculated)applyRouteImpactToStop(navMeta.id||navMeta.placeId,routeDurationBefore);
}
...
function startFullDayRoute(){
  if(ensureDaySettings(state.activeDay).kind==='stay'){toast('Dit is een verblijfsdag zonder dagroute');return;}
...
function navigateToNextStop(){
  if(ensureDaySettings(state.activeDay).kind==='stay'){toast('Dit is een verblijfsdag zonder volgende routestop');return;}
...
function bind(){
  ...
  document.addEventListener('click',e=>{
    ...
    if(addStop){void addStopToActiveDay(addStop.dataset.stopCat, Number(addStop.dataset.addStop)); return;}
    ...
    const moveRound=e.target.closest('[data-move-roundtrip-stop]');
    if(moveRound){moveRoundtripStop(Number(moveRound.dataset.roundtripIndex),moveRound.dataset.moveRoundtripStop);return;}
    ...
    if(e.target.closest('#searchManualPlace')){void searchManualPlaces();return;}
    const manualResult=e.target.closest('[data-select-manual-place]');
    if(manualResult){selectManualPlace(Number(manualResult.dataset.selectManualPlace));return;}
    if(e.target.closest('#pickManualPlaceOnMap')){closeManualPlaceModal();beginMapPick('manual-place');return;}
    if(e.target.closest('#checkManualPlace')){checkManualPlaceLocation();return;}
    if(e.target.closest('#saveManualPlace')){saveManualPlace();return;}
    if(e.target.closest('#addDayAfter')){insertTripDayAfter(state.activeDay);return;}
    if(e.target.closest('#addStayDay')){insertTripDayAfter(state.activeDay,{stay:true});return;}
    if(e.target.closest('#unlinkPreviousOvernight')){unlinkPreviousOvernightFromDay(state.activeDay);return;}
    ...
    if(skipPlanStop){e.preventDefault(); e.stopPropagation(); void toggleSkipPlannedStop(Number(skipPlanStop.dataset.skipPlanStop)); return;}
    const movePlan=e.target.closest('[data-move-plan-stop]');
    if(movePlan){e.preventDefault();e.stopPropagation();void movePlanStop(Number(movePlan.dataset.planMoveIndex),movePlan.dataset.movePlanStop);return;}
    ...
    if(remove){const row=remove.closest('[data-plan-index]'); void removePlanRow(Number(row.dataset.planIndex));}
  });
  document.addEventListener('input',e=>{
    if(e.target.id==='manualPlaceSearch'){
      if(manualSelectedPlace){clearManualPlaceSelection();renderManualPlaceResults();}
      setManualSearchStatus('Klik op Zoeken om mogelijke plekken te laden.');return;
    }
    if(e.target.id==='manualPlaceLocation'){manualSelectedPlace=null;manualResolvedCenter=null;const save=$('#saveManualPlace');if(save)save.disabled=true;setManualPlaceStatus('Locatie gewijzigd — controleer opnieuw.');return;}
    ...
    if(e.target.classList.contains('plan-time-input')){
      ...
      if(i===0){
        ensureDaySettings(state.activeDay).depart=normalizeTimeValue(e.target.value);
        if(state.activeDay===1){state.depart=ensureDaySettings(1).depart;if($('#departTime'))$('#departTime').value=state.depart;}
        refreshDayTimelineTimes(state.activeDay,{sortStops:false});
        syncFollowingDayArrival(state.activeDay);
      } else {
  ...
  document.addEventListener('change',e=>{
    if(e.target.id==='manualPlaceRole'){syncManualPlaceRoleAndType('role');return;}
    if(e.target.id==='manualPlaceType'){syncManualPlaceRoleAndType('type');return;}
    if(e.target.id==='activeDayDepart'){setDayDepart(state.activeDay,e.target.value,{syncForm:true});return;}
    if(e.target.id==='activeDayDesiredArrival'){setDayDesiredArrival(state.activeDay,e.target.value);return;}
    if(e.target.id==='activeDayArrivalSlot'){setDayArrivalSlot(state.activeDay,e.target.value,{syncForm:true});return;}
  ...
  document.addEventListener('keydown',e=>{
    if(e.key==='Enter'&&e.target?.id==='manualPlaceSearch'){e.preventDefault();void searchManualPlaces();}
  });

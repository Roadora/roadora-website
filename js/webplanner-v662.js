...
function createDefaultState(){
  return {
    ...
    routeSource:'empty', routeDistanceKm:0, routeDurationMin:0, routeZones:[], placeStatus:{}, dayHotels:{}, routePreference:'fastest', routeVariantId:'', routeVariantSummaries:[], routeMismatchCount:0, foodType:'restaurant', outingType:'highlights', tripType:'single', roundtripStops:[], foodSearchRadiusKm:10, foodSearchCenter:null, outingSearchMode:'place', outingSearchRadiusKm:25, outingSearchQuery:'', outingSearchCenter:null
  };
}
...
function setFormFromState(){
  ...
  set('outingSearchQuery', state.outingSearchQuery || '');
...
function pointWithRouteMeta(center,index=0){
  ...
  return {lat,lng,index,progress:projection?clampRouteProgress(projection.progress):null,distanceFromStartMeters:projection?Math.round(clampRouteProgress(projection.progress)*Number(state.routeDistanceKm||0)*1000):null};
}
...
async function searchOutingsManually(){
  try{
    ...
    if(state.outingSearchMode==='current') center=await currentPosition();
    if(state.outingSearchMode==='place'){
      const q=sanitizePlainText($('#outingSearchQuery')?.value||state.outingSearchQuery,180);
      if(!q) throw new Error('Vul eerst een plaats of adres in');
      state.outingSearchQuery=q; center=await resolveSearchCenter(q); center.label=center.label||q;
    }
    ...
    const radiusKm=Math.max(5,Math.min(50,Number($('#outingSearchRadius')?.value||state.outingSearchRadiusKm)||25));
    state.outingSearchRadiusKm=radiusKm;
    await loadLivePlacesFor('uitjes',{points:[pointWithRouteMeta(state.outingSearchCenter)],radiusMeters:radiusKm*1000,maxResults:20,mode:`manual_${state.outingSearchMode}`});
...
async function loadLivePlacesFor(cat,options={}){
  ...
  try{
    ...
    if(cat==='uitjes') body.outingType=state.outingType||'highlights';
    setPlaceStatus(cat,'loading',cat==='restaurants'?'Eten binnen 10 km laden…':cat==='uitjes'?'Uitjes rond je gekozen gebied laden…':'Live resultaten laden…');
    ...
    stops[cat]=list;
    if(list.length){
      const message=cat==='restaurants'?`${list.length} opties binnen 10 km van je actuele locatie.`:cat==='uitjes'?`${list.length} uitjes rond het door jou gekozen gebied.`:(isOvernightCategory(cat)?`${list.length} resultaten binnen het gekozen aankomstgebied.`:`${list.length} resultaten langs Dag ${state.activeDay}.`);
      setPlaceStatus(cat,'live',message);renderStops();return true;
    }
    setPlaceStatus(cat,'empty',cat==='restaurants'?'Geen eten gevonden binnen 10 km. Probeer een andere categorie.':cat==='uitjes'?'Geen uitjes gevonden binnen deze straal. Vergroot de straal of kies een ander gebied.':isOvernightCategory(cat)?'Geen resultaten binnen 22 km van het gekozen aankomstgebied.':'Geen live resultaten gevonden.');renderStops();
...
function readForm(){
  ...
  state.outingSearchQuery=sanitizePlainText($('#outingSearchQuery')?.value||state.outingSearchQuery,180);
}
...
function stopMatchLabels(cat, desc='', meta={}){
  ...
  if(cat==='uitjes') return [meta.searchAreaLabel||'Zelf gekozen zoekgebied',availabilityLabel(meta)||'opening controleren',detour].slice(0,3);
...
function renderStops(){
  ...
  if(controls){
    if(state.category==='restaurants'){
      ...
      controls.classList.remove('hidden');controls.setAttribute('aria-hidden','false');
      controls.innerHTML=`<div class="manual-search-card outing-search-card"><strong>Waar wil je zoeken?</strong><div class="search-mode-row">${[['current','Huidige locatie'],['place','Plaats/adres'],['map','Kaartpunt'],['route','Routepunt']].map(([id,label])=>`<button type="button" class="search-mode ${state.outingSearchMode===id?'active':''}" data-outing-search-mode="${id}">${label}</button>`).join('')}</div><div class="outing-search-fields"><input id="outingSearchQuery" class="${state.outingSearchMode==='place'?'':'hidden'}" value="${escapeHtml(state.outingSearchQuery||'')}" placeholder="Bijv. Heidelberg of exact adres"><select id="outingSearchRadius" aria-label="Zoekstraal uitjes"><option value="10" ${Number(state.outingSearchRadiusKm)===10?'selected':''}>10 km</option><option value="25" ${Number(state.outingSearchRadiusKm)===25?'selected':''}>25 km</option><option value="50" ${Number(state.outingSearchRadiusKm)===50?'selected':''}>50 km</option></select>${state.outingSearchMode==='map'||state.outingSearchMode==='route'?`<button type="button" class="btn" id="chooseOutingPoint">${state.outingSearchCenter?'Punt opnieuw kiezen':'Punt kiezen op kaart'}</button>`:''}<button type="button" class="btn primary" id="searchOutingsManual">Zoek uitjes</button></div><span class="manual-search-status">${escapeHtml(state.outingSearchCenter?.label||'Roadora zoekt alleen nadat jij een gebied hebt gekozen.')}</span></div>`;
...
function bind(){
  ...
  document.addEventListener('click',e=>{
    ...
    if(outingFilter){
      ...
      state.suggestions=false; state.view='all';
      stops.uitjes=[]; setPlaceStatus('uitjes','idle',`Kies zelf een gebied en zoek naar ${outingTypeLabel(nextType).toLowerCase()}.`);
  ...
  document.addEventListener('input',e=>{
    ...
    if(e.target.id==='outingSearchQuery'){state.outingSearchQuery=sanitizePlainText(e.target.value,180);scheduleAutosave();return;}

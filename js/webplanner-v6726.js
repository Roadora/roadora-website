...
function openManualPlaceModal(){const modal=$('#manualPlaceModal'); if(!modal)return; const day=$('#manualPlaceDay');if(day)day.textContent=String(state.activeDay);manualResolvedCenter=null;setManualPlaceStatus('Nog geen locatie gecontroleerd.');modal.classList.add('open');modal.setAttribute('aria-hidden','false');}
...
async function checkManualPlaceLocation(){
  const location=sanitizePlainText($('#manualPlaceLocation')?.value,400);if(!location){toast('Vul eerst een adres, link of coördinaten in');return null;}
  setManualPlaceStatus('Locatie controleren…','loading');
  try{
    const center=manualResolvedCenter?.input===location?manualResolvedCenter:await resolveSearchCenter(location);
    ...
    const routeText=projection?` · ${Math.round(projection.gapKm*10)/10} km van de route`:'';
...
async function saveManualPlace(){
  ...
  try{
    ...
    const projection=hasRoute()?nearestRouteProjection(center.lat,center.lng):null;
    if(projection&&projection.gapKm>25){
      const totalExtra=Math.max(10,Math.round(4+projection.gapKm*4.4));
      if(!confirm(`Deze plek ligt ongeveer ${Math.round(projection.gapKm)} km van je gekozen route. Geschatte totale extra reistijd: ongeveer ${durationLabel(totalExtra)}. Toch toevoegen?`)) return;
    }
    ...
    const meta={id:`user-${Date.now()}`,source:'user',manualLabel:label,userAdded:true,provider:'Zelf toegevoegd',lat:Number(center.lat),lng:Number(center.lng),placeId:center.placeId||'',name,category:cat,address:center.label||location,notes,sourceUrl:isGoogleMapsUrl(location)?location:'',resolvedAt:new Date().toISOString(),googleMapsUri:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${center.lat},${center.lng}`)}`,status:'controleer toegang en beschikbaarheid bij de aanbieder',amenities:['Zelf toegevoegd'],stopDurationMin:type==='food'?45:type==='outing'?60:(type==='hotel'||type==='camperplace'?0:10)};
    ...
    state.category=cat; manualResolvedCenter=null; closeManualPlaceModal(); await addStopToActiveDay(cat,index);
...
async function addStopToActiveDay(cat,index){
  ...
  if(isOvernightCategory(cat)){
    ...
    await recalculateRouteThroughSelectedStops('overnachting toegevoegd');
    return;
  }
  ...
  await recalculateRouteThroughSelectedStops(Number.isInteger(replacement)?'stop vervangen':'stop toegevoegd');
}
...
function bind(){
  ...
  document.addEventListener('input',e=>{
    if(e.target.id==='manualPlaceLocation'){manualResolvedCenter=null;setManualPlaceStatus('Locatie gewijzigd — controleer opnieuw.');return;}
